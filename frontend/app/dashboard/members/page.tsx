...
export default function MembersPage() {
  ...
  const selectedRole = useSignal<"admin" | "member" | "teacher">("member");
  ...
  useEffect(() => {
    if (!profileLoading.value && profile.value?.role !== "admin") {
  ...
  }, [profileLoading.value, profile.value?.role, fetchRequests, router]);
  ...
  async function handleApprove() {
    ...
    try {
      ...
      const name = [request.user?.first_name, request.user?.last_name].filter(Boolean).join(" ") || request.user?.email || "User";
      toast.success(`${name} has been approved as ${selectedRole.value}.`);
    } catch (err) {
  ...
  async function handleReject(request: JoinRequest) {
    actionLoading.value = true;
    try {
      await api(`/schools/join-requests/${request.id}/reject`, { method: "PATCH" });
      requests.value = requests.value.filter((r) => r.id !== request.id);
      const name = [request.user?.first_name, request.user?.last_name].filter(Boolean).join(" ") || request.user?.email || "User";
  ...
  const isLoading = loading.value || profileLoading.value;

  return (
    <div className="space-y-6">
      <DashboardPageHeader
        title="Members"
        description="Review and manage join requests for your school"
      />
          ...
          {requests.value.map((request) => {
            const name =
              [request.user?.first_name, request.user?.last_name]
                .filter(Boolean)
                .join(" ") || request.user?.email || "Unknown user";
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Assign a role to{" "}
              {[
                selectedRequest.value?.user?.first_name,
                selectedRequest.value?.user?.last_name,
              ]
                .filter(Boolean)
                .join(" ") ||
                selectedRequest.value?.user?.email ||
                "this user"}
              .
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2 py-2">
            ...
            <select
              ...
              onChange={(e) =>
                (selectedRole.value = e.target.value as "admin" | "member" | "teacher")
              }
            >
